...
class Polygon implements Primitive {
	...
	build(instance: SceneInstance): BuiltGeometry {
		...
		if (count >= 3) {
			...
			const first = transformPoint(firstPoint, instance.position)
			for (let i = 1; i < count - 1; i += 1) {
				...
				const second = transformPoint(secondPoint, instance.position)
				const third = transformPoint(thirdPoint, instance.position)
