...
class DodecahedronScene implements FigureScene {
	...
	constructor({
		...
		this.geometry = DodecahedronScene.createDodecahedronGeometry(scale)
		this.lightDirection = lightDirection
	}

	private static createIcosahedronVertices(): Vec3[] {
	...
	private static createIcosahedronFaces(): number[][] {
		return [
			[0, 11, 5],
			[0, 5, 1],
			[0, 1, 7],
	...
	private static getFaceCenter(vertices: Vec3[], face: number[]): Vec3 {
		let sum: Vec3 = [0, 0, 0]
	...
	private static sortAdjacentCenters(
		icosaVertex: Vec3,
		centers: {
			index: number,
			center: Vec3,
	...
	private static assignFaceColors(facesAsPentagons: number[][]): Vec4[] {
		...
		for (let faceIndex = 0; faceIndex < facesAsPentagons.length; faceIndex++) {
			...
			for (let i = 0; i < face.length; i++) {
				...
				const edgeKey = DodecahedronScene.makeEdgeKey(a, b)
		...
		for (let faceIndex = 0; faceIndex < facesAsPentagons.length; faceIndex++) {
			...
			for (let paletteIndex = 0; paletteIndex < DodecahedronScene.FACE_PALETTE.length; paletteIndex++) {
		...
		return colorIds.map(index => DodecahedronScene.FACE_PALETTE[index]!)
	}

	private static createDodecahedronGeometry(scale = 1.6): DodecahedronGeometry {
		const icosaVertices = DodecahedronScene.createIcosahedronVertices()
		const icosaFaces = DodecahedronScene.createIcosahedronFaces()
		const dodecaVertices = icosaFaces.map(face => DodecahedronScene.getFaceCenter(icosaVertices, face))

		const incidentFaces = new Map<number, {
			index: number,
			center: Vec3,
		}[]>()
		...
		for (let vertexIndex = 0; vertexIndex < icosaVertices.length; vertexIndex++) {
			...
			const orderedFaceIndices = DodecahedronScene.sortAdjacentCenters(icosaVertices[vertexIndex]!, centerList)
			...
			const normal = vec3Normalize(
				vec3Cross(
					vec3Subtract(second, first),
					vec3Subtract(third, first),
				),
			)
			const center = vec3Normalize(
				vec3Scale(
					orderedFaceIndices.reduce<Vec3>(
						(acc, faceId) => vec3Add(acc, dodecaVertices[faceId]!), [0, 0, 0],
					),
		...
		const faceColors = DodecahedronScene.assignFaceColors(facesAsPentagons)
		const dodecaFaces: DodecahedronGeometry['faces'] = []
		for (let faceIndex = 0; faceIndex < facesAsPentagons.length; faceIndex++) {
		...
		for (const pentagon of facesAsPentagons) {
			for (let i = 0; i < pentagon.length; i++) {
				edgeSet.add(DodecahedronScene.makeEdgeKey(
					pentagon[i]!,
					pentagon[(i + 1) % pentagon.length]!,
				))
			}
		}
